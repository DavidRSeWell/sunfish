...
class ChessData:
    def __init__(self, data_path: str, min_game_length: int = 10) -> None:
        self.min_game_length = min_game_length
        self.sequences = self.get_sequences(data_path)
    def filter_data(self, seq):
        return [s for s in seq if len(s) > self.min_game_length]
    def get_sequences(self, data_path):
        with open(data_path, "r") as f:
            data = f.readlines()
        return self.filter_data([s.replace("\n", "").split(" ") for s in data])
    def __len__(self):
...
class CharDataset(Dataset):
    ...
    @classmethod
    def load_chess_data(cls, data):
        chars = [-100] + list(set(list(itertools.chain.from_iterable(data))))
